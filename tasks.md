...
Статус на 2026-09-05: пункты 1–8 сделаны (коммиты «Корзина, оформление заказа
и страницы-хабы по макетам» и «Рефактор: общие миксины, чистка комментариев,
семантика заголовков»). Дополнительно исправлена семантика заголовков: слоган
в шапке был `<h1>` на каждой странице.

Вопросы, которые нужно решить отдельно (вынесены в конец, чтобы не блокировать работу):
- SEO: ведём разметку в теме или ставим плагин (Yoast/Rank Math)? Сейчас тема
  печатает description/OG/JSON-LD сама и молча отключается, если плагин найден.
- SEO: на главной `<h1>` скрыт визуально (в макете текстового заголовка нет).
  Альтернатива — добавить видимый заголовок в hero.
- Спам: лимит «2 отправки с IP за 15 минут» может задеть школу/офис с общим
  внешним адресом. Оставляем так или смягчаем IP-лимит, опираясь на лимит по
  номеру телефона?
- Оплата: на стенде не подключён ни один способ оплаты, поэтому заказ нельзя
  довести до конца — нужно решить, какой шлюз ставим.