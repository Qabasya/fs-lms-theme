...
- SEO ведём **плагином** (Yoast/Rank Math). Модуль темы `inc/Seo.php` остаётся
  как запасной вариант: он сам замолкает, как только плагин установлен, поэтому
  дублей мета-тегов не будет, а до установки плагина страницы не остаются без
  description/OG.
- `<h1>` главной — «ЕГЭ, ОГЭ, программирование и робототехника в Калининграде»
  (скрыт визуально, в макете такого заголовка нет; на остальных страницах
  заголовок свой).
- Лимит спама оставляем как есть: 2 отправки с IP за 15 минут + 2 с одного
  номера телефона в час.
- Оплата настраивается только на рабочем сайте, на стенде шлюзов нет.