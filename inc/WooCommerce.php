...
/**
 * 2026-09-14 (по указанию пользователя): любой товар продаётся только в
 * одном экземпляре — абонемент на месяц не покупают «три штуки».
 *
 * Тот же эффект, что галочка «Продавать по одному» во вкладке «Запасы»
 * товара, но сразу для всех, включая будущие товары. `WC_Product::
 * get_max_purchase_quantity()` для таких товаров отдаёт 1, и при min = max
 * = 1 `woocommerce_quantity_input()` выводит скрытое поле вместо счётчика —
 * и на странице товара, и в корзине. `WC_Cart::add_to_cart()` сам не даёт
 * положить второй такой же товар («Вы не можете добавить ещё один…»).
 */
add_filter( 'woocommerce_is_sold_individually', '__return_true' );

/**
 * Карточка товара по макету: бейдж категории над заголовком, короткое
 * описание, общий футер «цена + кнопка».
 *
 * Ссылку карточки закрываем сразу после заголовка (в дефолте — после
 * цены): кнопка «В корзину» не должна оказаться внутри `<a>`.
 */
...
/**
 * Миниатюра товара — 4:3, а не квадрат по умолчанию.
 *
 * BugFix (2026-09-12): картинка в карточке витрины обрезалась дважды.
 * WooCommerce кропит `woocommerce_thumbnail` до 300×300 (`crop => 1`),
 * а карточка выводит её в контейнере `aspect-ratio: 4 / 3` с
 * `object-fit: cover` (`src/scss/_woocommerce.scss`) — квадрат
 * масштабировался по ширине, и верх с низом уезжали за край.
 *
 * Исходники направлений (`img/shop-*.png`) — 1920×1440, ровно 4:3:
 * согласуем размер миниатюры с сеткой, и кроп перестаёт что-либо
 * отрезать. `crop => 1` оставляем сознательно: у произвольной картинки
 * из медиатеки пропорции любые, и без кропа карточки в ряду разъехались
 * бы по высоте — здесь кроп приводит их к единому 4:3 по центру.
 *
 * 600×450 (двойной размер к ~300px ширины карточки) — под экраны с
 * удвоенной плотностью пикселей, иначе на ретине картинка мылит.
 *
 * После изменения размеров миниатюры нужно перегенерировать уже
 * загруженные: Инструменты → Regenerate Thumbnails либо
 * `wp media regenerate`, иначе старые 300×300 останутся в uploads.
 */
...
/**
 * Фолбэк-картинка товара по категории направления (`img/shop-*.png`) —
 * настоящее фото по-прежнему ставится featured image в админке.
 *
 * Фильтруем именно `woocommerce_placeholder_img` (готовый `<img>`), а не
 * `..._img_src`: второй вызывается только когда в настройках нет своего
 * плейсхолдера, и на сайтах с настроенным его не видно вовсе.
 */
/**
